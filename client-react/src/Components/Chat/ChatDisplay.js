...
function ChatDisplay(props) {
    ...
    // the upload widget is defined only once on mount.
    ...
    // changes firebase timestamps to date (hh:mm)
    ...
    // registers the image/video in firebase with the respective type and label its text as the image_url.
    ...
    // upon sending a message, if the chatList document exists, we will update the recent texted user using timestamps,
    // if it doesnt exist, we will create a new chatList document & user collection and set the recent texted user in
    // the chatList document (this process will happen on both, the sender and the receiver sides).
    // the sent message will also get added on both sender and receiver sides as well.
    ...
    // if the user if focused on a chat, this function will listen to live updates from the db 
    // (real-time chatting using onSnapshot)
    function messagesListenRead() {
        if (props.to) {

            // this is the first filtering process. It fetches all the messages sent to the current logged in user or 
            // the target user (the one you're texting with). This simply means that we will target all messages 
            // that are sent to us alongside the messages that are sent to the user we want to text with.
            // this will only be 1/2 the filtering process as these messages will contain messages sent to us
            // but not from the user we are focused on. (firebase doesn't support multiple in queries at the same time)
            ...
            // the second filtering process happens here. This process will make sure to only target messages
            // that are sent either from us (the logged in user) or the user we want to text with (target user)
            // this fills up the hole that the first filtering process left and completes the filtering.
            ...
            // sorting the messages by most recent messages sent
            ...
            // a second scroll to make sure we scroll to the latest message
            setTimeout(() => {
                if (scrollRef.current) {
                scrollRef.current.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'start' })
                }
        ...
        <div className="chat-display-wrap">
            ...
            {!loading ? Object.keys(toCreds).length > 0 ?
                ...
                <div style={{display: "flex", flexDirection: "row", alignItems: "center"}}>
                    ...
                    <div className="fade-in" style={{display: "flex", flexDirection: "column", justifyContent: "center", marginRight: "30px", minWidth: "100px"}}>
